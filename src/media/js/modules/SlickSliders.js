...
function initSlider($container) {
	...
	let $intro_slide = $container.find('.intro-slide');
	...
	if (totalSlides > 0) {
		...
		if ($dotsContainer.length) {
			let $dotModel = $dotsContainer.children().first().removeClass(ACTIVE);
			$dotsContainer.empty();
			...
			$slides.on('beforeChange', function (e, slick, currentSlide, nextSlide) {
				...
				for (let k = 0; k < $dotsContainer.length; k++) {
					...
					let $currentDot = $currentDotsContainer.children().eq(nextSlide).addClass(ACTIVE);
				}
			});

			for (let k = 0; $dotsContainer.length > k; k++) {
				$dotsContainer.eq(k).children().first().addClass(ACTIVE);
			}
		}
		...
		$intro_slide.eq(0).addClass(ACTIVE);
		$slides.on('beforeChange', function (e, slick, currentSlide, nextSlide) {
			$intro_slide.removeClass(ACTIVE).eq(nextSlide).addClass(ACTIVE);
		});
...
function scrollTo(slider, position, immediate) {
	let $slider = $(slider);
	$slider.slick('setPosition');
	$slider.slick('slickGoTo', parseInt(position), immediate);
}
function add(slider, template) {
